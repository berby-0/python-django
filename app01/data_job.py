...
class HandleLagouData(object):
    ...
    def get_joblist(self):
        data = models.Job.objects.filter(keyword=self.key_word)
    ...
    def query_industryfield_result(self):
        info = {}
        data = models.Job.objects.filter(keyword=self.key_word).values('companyind_text')
    ...
    def query_salary_result(self):
        info = {}
        data = models.Job.objects.filter(keyword=self.key_word).values('providesalary_text')
        ...
        dictnum = {}
        for i in range(len(name_list3)):
            if name_list3[i] in dictnum.keys():
                dictnum[name_list3[i]] += 1
            else:
                dictnum.setdefault(name_list3[i], 1)
        maxnum = 0
        maxkey = 0
    ...
    def query_workyear_result(self):
        info = {}
        data = models.Job.objects.filter(keyword=self.key_word).values('workyear')
        data = [x['workyear'] for x in data]
        data = np.char.replace(data, '','0')
        #数据转换
        d = [list(data_list.workyear_l.values())[list(data_list.workyear_l.keys()).index(i)] for i in data]
        result = pd.Series(d)
        # 处理原始数据
    ...
    def query_education_result(self):
        info = {}
        data = models.Job.objects.filter(keyword=self.key_word).values('degreefrom')
        data = [x['degreefrom'] for x in data ]
        data = np.char.replace(data, '','0')
        #数据转换
        d = [list(data_list.degreefrom_l.values())[list(data_list.degreefrom_l.keys()).index(i)] for i in data]
        result = pd.Series(d)
    ...
    def query_city_result(self):
        info = {}
        data1 = models.Job.objects.filter(keyword=self.key_word).values('workarea_text', 'providesalary_text')
    ...
    def query_financestage_result(self):
        info = {}
        data = models.Job.objects.filter(keyword=self.key_word).values('companytype_text')
    ...
    def query_companysize_result(self):
        info = {}
        data = models.Job.objects.filter(keyword=self.key_word).values('companysize_text')
    ...
    def query_tag_result(self):
        ...
        wordcloud.to_file('static/img/job_key.jpg')
        plt.axis("off")
        return data1

class HandleLagouData2(object):
    def __init__(self):
        self.date = time.strftime("%Y-%m-%d",time.localtime())
        self.key_word = '北京'
    ...
    def get_joblist(self):
        data = models.Job.objects.filter(workarea_text__startswith=self.key_word)
        data = [{"job_href": x.job_href, "job_name": x.job_name,
                          "companytype_text": x.companytype_text, "jobwelf_list": x.jobwelf_list,
                          "degreefrom": x.degreefrom, "company_name": x.company_name,
                          "workyear": x.workyear, "companysize_text": x.companysize_text,
    ...
    def query_industryfield_result(self):
        info = {}
        data = models.Job.objects.filter(workarea_text__startswith=self.key_word).values('companyind_text')
        data = [x['companyind_text'] for x in data if x['companyind_text'] != '']
        list = []
        for i in data:
            for j in i.split('/'):
    ...
    def query_salary_result(self):
        info = {}
        data = models.Job.objects.filter(workarea_text__startswith=self.key_word).values('providesalary_text')
        data = [avg(x['providesalary_text']) for x in data if x['providesalary_text'] != '']
        name_list = []
        for i in range(len(data)):
            name_list.append(i+1)
    ...
    def query_workyear_result(self):
        info = {}
        data = models.Job.objects.filter(workarea_text__startswith=self.key_word).values('workyear')
        data = [x['workyear'] for x in data]
        data = np.char.replace(data, '','0')
        #数据转换
        d = [list(data_list.workyear_l.values())[list(data_list.workyear_l.keys()).index(i)] for i in data]
    ...
    def query_education_result(self):
        info = {}
        data = models.Job.objects.filter(workarea_text__startswith=self.key_word).values('degreefrom')
        data = [x['degreefrom'] for x in data ]
        data = np.char.replace(data, '','0')
        #数据转换
        d = [list(data_list.degreefrom_l.values())[list(data_list.degreefrom_l.keys()).index(i)] for i in data]
    ...
    def query_financestage_result(self):
        info = {}
        data = models.Job.objects.filter(workarea_text__startswith=self.key_word).values('companytype_text')
        data = [x['companytype_text'] for x in data if x['companytype_text'] != '']
        result = pd.Series(data)
        # 处理原始数据
        result_list1 = [x for x in result ]
    ...
    def query_city_result(self):
        info = {}
        data1 = models.Job.objects.filter(workarea_text__startswith=self.key_word).values('workarea_text', 'providesalary_text')
        data = [[x['workarea_text'],x['providesalary_text']] for x in data1 if x['workarea_text'] != '异地招聘']
        list = []
        for i in data:
            x = 0
    ...
    def query_companysize_result(self):
        info = {}
        data = models.Job.objects.filter(workarea_text__startswith=self.key_word).values('companysize_text')
        data = [x['companysize_text'] for x in data if x['companysize_text'] != '']
        result = pd.Series(data)
        # 处理原始数据
        result_list1 = [x for x in result]
    ...
    def query_tag_result(self):
        info = {}
        data = models.Job.objects.filter(workarea_text__startswith=self.key_word).values('jobwelf_list')
        data = [x['jobwelf_list'] for x in data if x['jobwelf_list'] != '']
        result = pd.Series(data)
        # 处理原始数据
        data1 = str(data).replace("\"", "").replace(" ", "") \
